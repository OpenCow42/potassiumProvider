...
struct SnapshotGenerationPagingTests {
    ...
    @Test func migratesLegacySnapshotWithoutChangingItsContents() async throws {
        ...
        try await withStore(databaseURL: databaseURL) { store in
            ...
            #expect(snapshot.items.first?.isFavorite == nil)
    ...
    @Test func favoriteMetadataRoundTripsThroughSnapshotGenerations() async throws {
        try await withStore { store in
            let favorite = KDriveRemoteItem(
                id: 7,
                name: "Favorite.txt",
                type: "file",
                status: "ok",
            ...
            try await store.save(
                KDriveSnapshot(isFullyEnumerated: true, items: [favorite]),
                domainIdentifier: "domain",
                containerIdentifier: "42"
            )
            ...
            #expect(page.items.first?.isFavorite == true)
        }
    }

    @Test func legacyWorkingSetJSONDecodesWithoutFavoriteMetadata() async throws {
        let directory = temporaryDirectory()
        defer { try? FileManager.default.removeItem(at: directory) }
        let databaseURL = directory.appendingPathComponent("Snapshots.sqlite3")
        try await withStore(databaseURL: databaseURL) { store in
            let legacyItem = KDriveRemoteItem(
                id: 7,
                name: "Legacy.txt",
                type: "file",
                status: "ok",
            ...
            var json = try #require(
                JSONSerialization.jsonObject(with: JSONEncoder().encode(legacyItem)) as? [String: Any]
            )
            ...
            do {
                ...
                try database.run(
                    """
                    INSERT INTO working_set_poll_state(
                        domainIdentifier,
                        workingSetAnchor,
                        workingSetItemsJSON,
                        lastPollAttemptAt,
            ...
            let snapshot = try #require(try await store.workingSetSnapshot(domainIdentifier: "domain"))
            #expect(snapshot.items.first?.name == "Legacy.txt")
            #expect(snapshot.items.first?.isFavorite == nil)
        }
    }
    ...
    private func withStore<Result>(
        _ operation: (KDriveSnapshotSQLiteStore) async throws -> Result
    ) async throws -> Result {
        let directory = temporaryDirectory()
        defer { try? FileManager.default.removeItem(at: directory) }
