...
### 2026-09-15 integration with main

PR #6 was updated from `ac14eeb` by merging `main` at `7ba89bf`. The integration
retains Stability profile isolation and diagnostics, validates kDrive membership
on the external add path, preserves lab purpose/ownership during stable-identity
coding, and adapts setup/known-folder fixtures to the current registration flow.

Validation used Xcode 26.5 (17F42) on arm64 macOS 26.6.2 (25G83):

| Validation | Result |
| --- | --- |
| Standard macOS unit target | 454 passed |
| Final standard macOS known-folder fixture rerun | 6 passed |
| macOS setup navigation, storage-choice add, and removal-confirmation UI | 3 passed |
| Stability macOS unit target | 589 passed |
| iPhone 17, iOS Simulator 26.5 (23F73), unit target | 421 passed |
| Apple Vision Pro, visionOS Simulator 26.5 (23O470), unit target | 421 passed |
| Generic visionOS device build | Passed |

The test commands use `xcodebuild test -project potassiumProvider.xcodeproj`,
`-scheme potassiumProvider` (or `potassiumProvider-Stability`), and these exact
destinations:

- `platform=macOS,arch=arm64`
- `platform=iOS Simulator,OS=26.5,name=iPhone 17`
- `platform=visionOS Simulator,OS=26.5,name=Apple Vision Pro`

Standard unit runs select `-only-testing:potassiumProviderTests`. The macOS UI
selection adds `testSetupNavigatesFromAccountCellToAvailableDriveManagement`,
`testFakeOAuthLoginDiscoversAndAddsDriveToFileProvider`, and
`testConfiguredDrivePresentsRemovalConfirmation` under
`potassiumProviderUITests/potassiumProviderUITests`. Simulator commands also set
`CODE_SIGNING_ALLOWED=NO`; both macOS profiles use normal development signing.
Final result bundles and logs are retained locally under
`/tmp/potassium-pr6-validation/` (`mac-verified`, `mac-stability-verified`, `ios`,
`visionos`, and `mac-known-folders`). The generic build used `xcodebuild build`
with `-destination 'generic/platform=visionOS' CODE_SIGNING_ALLOWED=NO`; its log
is `visionos-device-build.log`. The earlier failed candidates are retained: one new test
incorrectly rejected read-only registration queries, and three known-folder
fixtures relied on the Stability profile registering an ordinary saved domain.
The assertions/fixtures were corrected without weakening production isolation.

No live kDrive or physical external-drive check ran. The physical matrix below
remains required for physical external-drive acceptance; existing conflict-safety
finding states remain unchanged.
